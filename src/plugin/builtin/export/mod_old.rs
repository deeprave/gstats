...
use crate::plugin::data_export::{PluginDataExport, DataExportType};
...
pub struct ExportPlugin {
    ...
    export_completed: Arc<RwLock<bool>>,
}
...
impl ExportPlugin {
    ...
    async fn format_data(&self, data: Vec<Arc<PluginDataExport>>, config: &ExportConfig) -> PluginResult<String> {
        // TODO: Implement schema-aware formatters
        // For now, use a simple debug format
        ...
        for export in data {
            ...
            // TODO: Format actual data based on schema
        }
            ...
            PluginEvent::DataReady { plugin_id, scan_id, export } => {
                ...
                // Add data to coordinator
                {
                    ...
                    if coordinator.is_complete() {
                        ...
                        self.perform_export(all_data).await.map_err(|e| {
                            NotificationError::processing(format!("Export failed: {}", e))
                        })?;
